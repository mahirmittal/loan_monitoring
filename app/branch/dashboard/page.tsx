...
import { getBranchAuth, clearBranchAuth } from "@/lib/branch-auth"

interface LoanApplication {
  id: string
  applicantName: string
  loanType: "individual" | "shg"
  address: string
  bankBranch: string
  description: string
  submittedAt: string
...
export default function BranchDashboard() {
  ...
  useEffect(() => {
    ...
    if (savedApplications) {
      ...
      // Filter applications for this specific branch
      const branchApplications = allApplications.filter(
        (app) => app.bankBranch === `${auth.bankName} - ${auth.branchName}`,
      )
      setApplications(branchApplications)
    }
    setIsLoading(false)
  }, [router])
  ...
  const handleDisburse = (applicationId: string) => {
    const updatedApplications = applications.map((app) => {
      if (app.id === applicationId && app.status === "approved") {
        return {
          ...app,
          status: "disbursed" as const,
          branchAction: {
            action: "disbursed" as const,
            timestamp: new Date().toISOString(),
            disbursedBy: branchAuth?.username || "Unknown",
          },
        }
      }
      return app
    ...
    const allApplications = JSON.parse(localStorage.getItem("loanApplications") || "[]")
    const updatedAllApplications = allApplications.map((app: LoanApplication) => {
      if (app.id === applicationId) {
        const updated = updatedApplications.find((ua) => ua.id === applicationId)
        return updated || app
      }
      return app
    })

    localStorage.setItem("loanApplications", JSON.stringify(updatedAllApplications))
  }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{branchAuth?.bankName}</h1>
              <p className="text-muted-foreground">{branchAuth?.branchName} - Branch Dashboard</p>
            </div>
          </div>
                            ...
                            {application.adminAction && (
                              <div className="bg-green-50 rounded-lg p-3">
                                <p className="text-sm text-green-800">
                                  ...
                                  {new Date(application.adminAction.timestamp).toLocaleString()}
                                </p>
                              </div>
                            )}
                          </div>
                            ...
                            {application.branchAction && (
                              <div className="bg-blue-50 rounded-lg p-3">
                                <p className="text-sm text-blue-800">
                                  ...
                                  {new Date(application.branchAction.timestamp).toLocaleString()} by{" "}
                ...
                {applications.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {applications.map((application) => (
                      <Card key={application.id} className="border-l-4 border-l-gray-300">
                        <CardContent className="pt-6">
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
